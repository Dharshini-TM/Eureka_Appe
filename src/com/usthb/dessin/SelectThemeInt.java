...
public class SelectThemeInt {
    ...
    public SelectThemeInt(Joueur joueur) {
        ...
        wn_choisetheme.setTitle("EUREKA | Choix du thème");
        ...
        wn_choisetheme.setLocation((int) screenSize.getWidth() / 2 - wn_choisetheme.getWidth() / 2,
                (int) screenSize.getHeight() / 2 - wn_choisetheme.getHeight() / 2);

        wn_choisetheme.addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosing(WindowEvent e) {
                JOptionPane.showMessageDialog(null, "Nous espérons que vous apprécierez nos thèmes", "EUREKA | EXIT",
        ...
        // THEME ISLAM - images and click handlers
        ...
        // Type-safe combo box
        JComboBox<ThemeType> comboBoxsujet = new JComboBox<>();
        ...
        btnChoise.addActionListener((ActionEvent selectTheme) -> {
            try {
                wn_choisetheme.setVisible(false);
                ThemeType chosen = (ThemeType) comboBoxsujet.getSelectedItem();
                if (chosen != null) {
                    joueur.Jouer(chosen);
                }
            } catch (Exception ex) {
